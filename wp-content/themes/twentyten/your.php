...
if(isset($_POST['update']))
{
	...
	else
	{
		...
		$sqlUP = "UPDATE G_Blog SET Title = '" . $txttitle . "', Content = '" . addslashes($txtcontent) . "'  WHERE IDBlog = " . $_GET["page_id"];
...
if(isset($_POST['insert']))
{
	...
	$target_dir2 = "images/";
	$first_file = $target_dir2 . basename($_FILES["firstfoto"]["name"]);

	// echo  $_FILES["firstfoto"]["name"];
	if($_FILES["firstfoto"]["name"] == "")
	{
		$sqlIN = "INSERT INTO G_Blog (Title, Content) VALUES ('Titel', 'Bericht')";
	}
	else
	{
		if(move_uploaded_file($_FILES["firstfoto"]["tmp_name"], $first_file))
		{
	 	 echo "Start FOTOINSERT is gelukt: " . $_FILES["firstfoto"]["name"];
		}
		else
		{
	 		echo "Niet gelukt";
		}
		$sqlIN = "INSERT INTO G_Blog (Title, Content, Photo) VALUES ('Titel', 'Bericht', '" . $first_file . "')";
...
/////// database fotoinvoeg
	...
	else
	{
		...
		$sqlUP = "UPDATE G_Blog SET Content = '" . addslashes($txtcontent) . "'  WHERE IDBlog = " . $_GET["page_id"];
...
// Firefox support
	...
	txtarea.value = front + '<img border="0" src="../images/' + fotonaam + '" style="max-width: 450px;">' + back;
}
function ShowMe()
{
...
<br /><br /><br />
Start foto: <br />
<input type="file" name="firstfoto" class="btn">
<br /><br /><br />
...
<textarea style="left: 50px" rows="10" cols="100" name="TxtContent" id="TxtContent" wrap="physical"><?php echo stripslashes($txtcontent); ?></textarea><br />
<br />

Foto tussen text: <input type="file" name="fileToUpload" id="fileToUpload" class="btn"> <input type="button" class="btn" name="fotonaam" value="Foto in text" onclick="IMGtoevoegen(document.getElementById('TxtContent'));" />
